...
export function analyzeLaunchSite(inclinationDeg: number, siteLat: number, siteLon: number) {
    const knownSites = [
...
export function debrisRiskImproved(altitudeKm: number, inclinationDeg: number) {
    let altScore = 0;
...
export function estimateLifetimeImproved(
    altitudeKm: number,
    massKg: number,
    areaM2: number,
    Cd = 2.2,
...
export function analyzeMissionPlanner(body: PlannerRequest) {
    const {
        siteLat,
        siteLon,
        altitudeKm,
