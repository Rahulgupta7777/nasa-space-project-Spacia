        ...
        >
            <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-semibold text-space-text-muted">Latest Solar Flare</h3>
                </div>
                {solarFlare.latestFlare.note && (
                    <div className="group relative">
                        <Info className="w-4 h-4 text-space-text-soft hover:text-yellow-400 cursor-help transition-colors" />
                        <div className="absolute right-0 top-6 w-64 p-3 bg-space-section border border-space-border rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 text-xs text-space-text-main">
            ...
            <div className="space-y-1.5 text-xs">
                <p className="text-space-text-soft">
                    <span className="text-space-text-muted">Region:</span> {solarFlare.latestFlare.activeRegion}
                </p>
                <p className="text-space-text-soft">
                    <span className="text-space-text-muted">Location:</span> {solarFlare.latestFlare.location}
                </p>
                {solarFlare.latestFlare.duration && (
                    <p className="text-space-text-soft">
                        <span className="text-space-text-muted">Duration:</span> {solarFlare.latestFlare.duration}
                    </p>
                )}
                {solarFlare.latestFlare.instruments && solarFlare.latestFlare.instruments.length > 0 && (
                    <p className="text-space-text-soft">
                        <span className="text-space-text-muted">Instruments:</span> {solarFlare.latestFlare.instruments.slice(0, 2).join(", ")}
